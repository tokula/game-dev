# Haltija — Finnish Roster

> See also: [haltija_finnish.md](haltija_finnish.md)

## Command hierarchy

Forest haltijat follow the most extensively documented internal structure:

```
Tapio (king of forests; rules over all forest life and the Tapiola realm)
  └── Mielikki (mistress of forests; Tapio's consort; governs healing and hunt outcome;
                  in practice prayed to more often than Tapio)
        ├── Tellervo (daughter; forest maiden, associated with clouds and grain)
        ├── Tuulikki (daughter; lesser documented)
        └── Nyyrikki (son; patron of hunters; marks paths through the forest)
```

No equivalent hierarchy is documented for household (*kotihaltija*), sauna (*saunatonttu*), water (*vedenhaltija*), or land (*maanhaltija*) types — each such haltija governs its domain independently with no attested superior.

> **Note:** Tapio and Mielikki occupy a position at the boundary between race-members and deity-tier beings. They are documented as haltijat (place-governing spirits) of the highest order, but also worshipped directly and invoked by name in Kalevala incantations. For game design purposes they can be treated as god-tier NPCs within the Haltija category rather than representative player-character archetypes.

**Source priority:** Primary sources (folklore, mythology, religious texts, historical accounts) are the authoritative origin. Games and fiction derive from them. Always populate `— Primary sources` first and most completely. Derivative sections exist for cross-reference only — they should never displace or reframe the original cultural record.

## Named individuals — Primary sources

| Name | Source | Rank / role | Notes |
|------|--------|-------------|-------|
| Mielikki | *Kalevala*; *Mythologia Fennica* | Mistress of forests; governs healing and hunt success | More frequently prayed to than Tapio; her generosity manifests as plentiful game; depicted in golden dress when favorable, unkempt when game is scarce |
| Nyyrikki | *Kalevala* | Son of Tapio and Mielikki; patron of hunters | Marks forest paths to aid hunters; name may derive from a bear-epithet root |
| Tapio | *Kalevala*; *Mythologia Fennica* | King of forests; lord of Tapiola | Referred to by euphemistic hunter's titles: "grey-beard", "king of the forest", "forest's old man"; worshipped at *tapionpöytä* spruce formations |
| Tellervo | *Kalevala* | Daughter of Tapio and Mielikki; forest maiden | Associated with clouds; described as a golden-haired forest maiden |
| Tuulikki | *Kalevala* | Daughter of Tapio and Mielikki | Less extensively documented; name related to *tuuli* "wind" |

## Individual distinctions — Primary sources

**Tapio**
- `[Specialization-signature]` Personifies and governs the entire forest realm (Tapiola) — authority over all forest life, not merely a local domain; this exceeds the standard haltija domain-bonding.
- `[Modifier]` Referred to only by euphemistic hunter's titles in ritual speech ("grey-beard", "king of the forest") — direct naming avoided during hunts. — *Finnish hunting folk belief, Suomen kansan vanhat runot*

**Mielikki**
- `[Specialization-major]` Active, proactive healing of injured animals — treats wounded paws, rescues fallen chicks, heals mating injuries in game birds; a healing specialization beyond general domain governance.
- `[Modifier]` Appearance tied to generosity: golden-dressed and beautiful when hunt is to be plentiful; unkempt and disheveled when she withholds game. — *Kalevala*

## Named individuals — Games

*IP filter: no qualifying entries identified. No public domain, CC0, or explicitly open-licensed game source documents named Haltija individuals beyond the primary-source names above.*

## Named individuals — Modern literary

*No qualifying entries identified.*

## Sources

**Provenance tags:** `[PD]` public domain · `[CC0]` Creative Commons Zero · `[CC-BY]` CC Attribution · `[CC-BY-SA]` CC Attribution-ShareAlike · `[CC-BY-NC]` CC Attribution-NonCommercial · `[CC-BY-NC-SA]` CC Attribution-NonCommercial-ShareAlike · `[CC-BY-NC-ND]` CC Attribution-NonCommercial-NoDerivatives · `[SRD]` D&D 5e SRD (CC-BY-4.0) · `[OGL]` Open Game License · `[GFDL]` GNU Free Documentation License · `[BLOCKED]` confirmed proprietary — do not use · append `*` when provenance is a best-guess rather than confirmed (e.g. `[PD*]`)

### Primary sources

- `[PD]` Kalevala, Elias Lönnrot (compiler), final edition 1849
- `[PD]` Mythologia Fennica, Christfrid Ganander, 1789
- `[PD]` Suomen kansan vanhat runot (Ancient Songs of the Finnish People), Finnish Literature Society (compiler), 1908–1948

### Secondary sources

- `[CC-BY-SA]` Wikipedia — Haltija, https://en.wikipedia.org/wiki/Haltija
- `[CC-BY-SA]` Wikipedia — Tapio (spirit), https://en.wikipedia.org/wiki/Tapio_(spirit)
- `[CC-BY-SA]` Wikipedia — Mielikki, https://en.wikipedia.org/wiki/Mielikki
