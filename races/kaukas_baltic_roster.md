# Kaukas — Baltic Roster

> See also: [kaukas_baltic.md](kaukas_baltic.md)

## Command hierarchy

Kaukai operate under the authority of Puskaitis (patron deity living underground beneath the elder bush), who directs their fertility and harvest work. No internal hierarchy among individual Kaukai is attested in primary sources. Within the homestead, each Kaukas acts as an independent guardian spirit.

Puskaitis → Kaukai / Barstukai → individual household assignments

**Source priority:** Primary sources (folklore, mythology, religious texts, historical accounts) are the authoritative origin. Games and fiction derive from them. Always populate `— Primary sources` first and most completely.

## Named individuals — Primary sources

No individual Kaukai are named in primary sources. Łasicki's 1582 account describes the category of spirit but identifies no individual members. They appear in folk belief as an unnamed collective attached to specific households.

## Individual distinctions — Primary sources

No named individuals to distinguish from the racial baseline.

## Named individuals — Games

*(Section omitted — no qualifying open-licensed entries.)*

## Named individuals — Modern literary

*(Section omitted — no notable named individuals in qualifying modern literary sources.)*

## Sources

**Provenance tags:** `[PD]` public domain · `[CC0]` Creative Commons Zero · `[CC-BY]` CC Attribution · `[CC-BY-SA]` CC Attribution-ShareAlike · `[SRD]` D&D 5e SRD (CC-BY-4.0) · `[OGL]` Open Game License · `[BLOCKED]` confirmed proprietary — do not use · append `*` when provenance is a best-guess rather than confirmed (e.g. `[PD*]`)

### Primary sources

- `[PD]` Jan Łasicki, *De diis Samagitarum caeterorumque Sarmatarum et falsorum Christianorum* (1582)
- `[PD]` 16th–17th century Christian missionary accounts of Lithuanian folk belief
- `[PD]` Lithuanian folk belief texts, compiled 19th century

### Secondary sources

- `[CC-BY-SA]` Wikipedia: "List of Lithuanian gods and mythological figures" — https://en.wikipedia.org/wiki/List_of_Lithuanian_gods_and_mythological_figures
